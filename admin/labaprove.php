...
<body>
	...
	<center><h1>Requested Laboratory Details</h1></center><BR><BR><BR><BR>
...
if(isset($_POST['submit']))
	{
		$a=$_POST['lno'];
		$q1=mysqli_query($con,"SELECT * FROM `lab` WHERE `licenceno`='$a'");
		while($row=mysqli_fetch_array($q1))
		{
			$q2=mysqli_query($con,"UPDATE `lab` SET `status`=1 WHERE `licenceno`='$a'");
			
		}
	}

$q=mysqli_query($con,"SELECT * FROM `lab` WHERE `status`=0");
while($row=mysqli_fetch_array($q))
{
	$lid=$row[0];
	echo "<tr><td>".$row[1]."</td><td>".$row[3]."</td><td>".$row[4]."</td><td>".$row[5]."</td><td>".$row[6]."</td><td>".$row[9]."</td><td>".$row[10]."</td><td>".$row[11]."</td><td><a href='approvelab.php?lid=$lid'>Aprove</a></td></tr>";
}
